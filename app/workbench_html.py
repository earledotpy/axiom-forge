  ...
  <script>
    ...
    async function renderPromotionReview(runId) {
      ...
      [
        ...
        ["Target name", preparation.target.name || "missing"],
        ["Target repository", preparation.target.repo || "missing"],
        ["Target base branch", preparation.target.base_branch || "missing"],
        ["Target base commit", preparation.target.target_base_sha || "missing"],
        ["Delegation base commit", preparation.target.delegation_target_base_sha || "missing"],
        ["Target base stale", preparation.target.stale_base ? "yes" : "no"],
        ["Run ID", preparation.run_id],
        ["Patch SHA-256", preparation.patch_sha256],
        ["Changed paths", (preparation.changed_paths || []).join(", ") || "none"],
        ["Verification evidence kind", preparation.verification.kind || "missing"],
        ["Verification evidence path", preparation.verification.path || "missing"],
        ["Verification result", preparation.verification.status || "missing"],
        ...(preparation.verification.reason ? [["Verification reason", preparation.verification.reason]] : []),
        ["Acceptance result", preparation.acceptance_result || "missing"],
