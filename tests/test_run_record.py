...
class RunRecordTests(unittest.TestCase):
    def test_build_command_writes_a_record_from_one_stdin_json_payload(self):
    ...
    def test_build_command_fails_with_one_reason_for_invalid_stdin_json(self):
        result = subprocess.run(
            [sys.executable, str(Path(__file__).parents[1] / "scripts" / "run_record.py"), "build"],
            input="not json",
            text=True,
    ...
    def test_target_mode_validation_requires_delegation_artifact_revision(self):
        record = {
            "run_id": "target-run",
            "base_sha": "abc123",
            "run_status": "COMPLETED",
    ...
    def test_target_mode_validation_rejects_malformed_delegation_artifact_revision(self):
        ...
        self.assertEqual(caught.exception.reason, "malformed_delegation_artifact_revision")
